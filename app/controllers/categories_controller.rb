class CategoriesController < ApplicationController
  ...
  # GET /categories.json
  def index
    @categories = Category.paginate(:page=>params[:page],:per_page=>APP_CONFIG[:per_page])
    if request.post?
      @name = params[:name]
      @categories = Category.where("name like ?","%#{@name}%").paginate(:page=>params[:page],:per_page=>APP_CONFIG[:per_page])
    end
  end
